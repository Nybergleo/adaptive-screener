...
IDENTITY_FIELDS = [
    ...
    FieldSpec(
        ...
        "text",
        "Bloomberg Industry Classification Standard level-1 grouping, the broadest classification label, used for sector-level attribution, benchmarking, and universe filtering. Input: security identifier plus BICS taxonomy level 1. Output units: string sector name.",
        default_input=False,
    ),
    FieldSpec(
        "industry_group",
        ...
        "text",
        "BICS level-2 grouping, one step more granular than sector, useful for peer group construction and more targeted factor analysis. Input: security identifier plus BICS taxonomy level 2. Output units: string industry-group name.",
        default_input=False,
    ),
]
...
FIELD_CATALOG = [
    FieldSpec(
        ...
        "oper_margin(fpt=LTM)",
        "numeric",
        "Operating profit as a fraction of revenue over the trailing twelve months; measures how efficiently a company converts sales into operating earnings before interest and tax. Input: security identifier plus fpt=LTM period selector. Output/filter units: percent points (25 = 25%).",
        default_min="10",
    ...
    FieldSpec(
        ...
        "numeric",
        "Same as operating margin but adds back amortization of intangibles, giving a cleaner view of operating profitability for companies with significant acquired intangible assets. Input: security identifier plus fpt=LTM period selector. Output/filter units: percent points (25 = 25%).",
        default_min="11",
        format_type="percent_points",
    ),
    FieldSpec(
        ...
        "numeric",
        "Operating profit before interest, tax, depreciation, and amortization as a fraction of revenue; a widely used proxy for cash generation capacity and the basis for most EV/EBITDA valuation multiples. Input: security identifier plus fpt=LTM period selector. Output/filter units: percent points (25 = 25%).",
        format_type="percent_points",
    ),
    FieldSpec(
        "margin_stability_pp",
    ...
    FieldSpec(
        ...
        "numeric",
        "Price divided by a time-weighted blend of the current and next fiscal year EPS estimates, calibrated to represent exactly 12 months of forward earnings regardless of fiscal year position. Input: security identifier plus fpt=BT and fpo=1 forward period selector. Output/filter units: dimensionless valuation multiple (22.4 = 22.4x).",
        default_max="25",
    ),
    FieldSpec(
        "ebit_ltm",
        ...
        "numeric",
        "Absolute operating profit in currency terms over the trailing twelve months; the numerator of operating margin, useful for sizing earnings power in comparable company analysis. Input: security identifier plus fpt=LTM period selector and selected currency. Output/filter units: millions of {currency}.",
    ),
    FieldSpec(
        "market_cap",
        ...
        "numeric",
        "Total equity value of the company at current market prices, calculated from shares outstanding times price; the most common measure of company size and the denominator in yield-based valuation metrics. Input: security identifier plus selected currency. Output/filter units: millions of {currency}.",
        default_min="500",
    ),
    FieldSpec(
        "turnover",
        ...
        "numeric",
        "Daily traded value of the security in currency terms; a liquidity measure used to assess how easily a position can be entered or exited without significant market impact. Input: security identifier plus selected currency. Output/filter units: millions of {currency}.",
    ),
    FieldSpec(
        "revenue_ltm",
        "LTM revenue",
        ...
        "numeric",
        "Total top-line sales over the trailing twelve months; the starting point for margin analysis and a key input to revenue-based valuation multiples such as EV/Sales. Input: security identifier plus fpt=LTM period selector and selected currency. Output/filter units: millions of {currency}.",
    ),
    FieldSpec(
        ...
        "free_cash_flow_yield",
        "numeric",
        "Free cash flow per share divided by price; expresses how much cash a company generates relative to its market value, analogous to an earnings yield but based on actual cash rather than accounting profit. Input: security identifier. Output/filter units: percent points (4 = 4%).",
        format_type="percent_points",
    ),
    FieldSpec(
        "dividend_yield",
        ...
        "dividend_yield",
        "numeric",
        "Annualized dividend per share divided by price; the income return component of total return, relevant for income-oriented screening and dividend discount valuation. Input: security identifier. Output/filter units: percent points (1.5 = 1.5%).",
        format_type="percent_points",
    ),
    FieldSpec(
        "return_on_capital",
        ...
        "return_on_capital_employed",
        "numeric",
        "Operating profit divided by capital employed (debt plus equity), measuring how productively the company deploys its total invested capital; a key indicator of competitive advantage and management quality. Input: security identifier. Output/filter units: percent points (322 = 322%). Uses return_on_capital_employed because return_on_capital does not resolve in BQL.",
        format_type="percent_points",
    ),
    FieldSpec(
        "sector_filter",
        ...
        "classification_name(BICS, 1)",
        "text",
        "Bloomberg Industry Classification Standard level-1 grouping, the broadest classification label, used for sector-level attribution, benchmarking, and universe filtering. Input: security identifier plus BICS taxonomy level 1. Output/filter units: string sector name. Text filters use exact equals or comma-separated in-list values.",
    ...
    FieldSpec(
        ...
        "classification_name(BICS, 2)",
        "text",
        "BICS level-2 grouping, one step more granular than sector, useful for peer group construction and more targeted factor analysis. Input: security identifier plus BICS taxonomy level 2. Output/filter units: string industry-group name. Text filters use exact equals or comma-separated in-list values.",
        default_output=False,
        format_type="text",
    ),
]
